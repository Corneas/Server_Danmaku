...
public class NetworkController{
	...
	private List<KeyData>[]	inputBuffer = new List<KeyData>[playerNum];
	private KeyData[]			keyData = new KeyData[playerNum];
    ...
    public NetworkController(string hostAddress, bool isHost) {
        ...
        for (int i = 0; i < inputBuffer.Length; ++i) {
            inputBuffer[i] = new List<KeyData>();
        }
    }
    ...
    void SendInputData()
	{
		...
		inputData.keyData = new KeyData[bufferNum];

		//string str1 = "Count :" + count;
		//Debug.Log(str1);
		for (int i = 0; i < count; ++i) {
			inputData.keyData[i] = inputBuffer[playerId][i];	// outofRange
    ...
    public void ReceiveInputData()
	{
		...
		for (int i = 0; i < inputData.count; ++i) {
			int frame = inputData.keyData[i].frame;
			//Debug.Log("Set inputdata"+i+"[" + recvFrame + "][" + frame + "]:" + (recvFrame + 1 == frame));
			if (recvFrame + 1 == frame) {
				inputBuffer[opponent].Add(inputData.keyData[i]);
				++recvFrame;
			}
		}
	...
	public bool EnqueueMouseData()
	{
		...
		KeyData keyData = m_inputManager.GetLocalKeyData();
		keyData.frame = sendFrame;

		inputBuffer[playerId].Add(keyData);        
	...
	public void DequeueMouseData()
	{
		...
		// 데이터가 모였으므로.
		for (int i = 0; i < playerNum; ++i) {
			keyData[i] = inputBuffer[i][0];
			inputBuffer[i].RemoveAt(0);

            //입력 관리자에게 동기된 데이터로서 전달합니다.
            m_inputManager.SetInputData(i, keyData[i]);
		}
